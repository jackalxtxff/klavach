<nav class="nav nav-underline">
    ...
    @if(count($dictionary->comments) > 0)
        <a class="nav-link {{request()->routeIs('dictionaries.showComments') ? 'text-dark' : 'text-secondary'}}" href="{{route('dictionaries.showComments', $dictionary)}}">Комментарии <span class="badge bg-light text-dark rounded-pill align-text-bottom">{{count($dictionary->comments)}}</span></a>
    @endif
</nav>
