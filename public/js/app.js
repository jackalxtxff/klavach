...
function ajaxFilter() {
  ...
  var newUri = "".concat(uri, "?chapter=").concat(data.chapter, "&type=").concat(data.type, "&search=").concat(data.search, "&page=").concat(data.page, "&sort=").concat(data.sort, "&direction=").concat(data.direction);
  history.replaceState(null, null, newUri);
  console.log(data, uri, newUri);
  $.ajax({
    url: "".concat(uri),
...
$('.type-dict').click(function () {
  var textarea = $('.textarea-subtitle');
...
$('.form-dictionary').submit(function (e) {
  e.preventDefault();
  var method = $(this).hasClass('create') ? 'POST' : 'PUT';
...
$('.rate-container input[name="rating"]').click(function () {
  var method = $('.rate-container').data('method');
  var uri = $('.rate-container').data('uri');
  var data = {
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      if (response.lastname) {
        ...
        var newUri = window.location.href.replace(response.lastname, name);
        history.replaceState(null, null, newUri);
        $('a').each(function () {
          var href = $(this).attr('href');
          var result = href.replace(response.lastname, name);
...
function cropperInit() {
  ...
  setTimeout(function () {
    cropper = new Cropper(image, {
      aspectRatio: 1 / 1,
      preview: '.img-preview' // ready: function () {
...
$('.send-update-comment').click(function (e) {
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      $(document).find(".small-comments-container .comment-block[data-id=\"".concat(data.comment.id, "\"")).replaceWith(data.view);
...
$('.comments').on('click', '.delete-comment', function (e) {
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      $('.small-comments-container').find(".comment-block[data-id=\"".concat(data.comment, "\"")).remove();
    },
    error: function error(data) {
      var response = data.responseJSON;
      console.log(response);
...
function recordsFilter() {
  var page = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 1;
  var uri = $('.dictionary-records-container').data('uri');
  var data = {
    sort: $('input[name="sortRecords"]:checked').data('value'),
    page: page
  };
  var newUri = "".concat(uri, "?sort=").concat(data.sort, "&page=").concat(data.page);
  history.replaceState(null, null, newUri);
  console.log(data, uri, newUri);
  $.ajax({
    url: "".concat(uri),
...
var playBtn = false; //Получение данных словаря при нажатии на кнопку начала
...
$('.play-game-btn').click(function (e) {
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      console.log(data);
      var arrExcerpt = new Array();

      for (var i = 0; i < data.excerpts.length; i++) {
        arrExcerpt.push(data.excerpts[i].excerpt.split(' '));
      }

      arrExcerpt = (_ref = []).concat.apply(_ref, _toConsumableArray(arrExcerpt));
      arrExcerpt = arrExcerpt.map(function (item) {
        return "".concat(item, " ");
      });
...
function GameInit(arrExcerpt, dictionary) {
      ...
      if (_Seconds === 3) {
        ...
        $.each(arrExcerpt, function (key, value) {
          if (key === arrExcerpt.length - 1) {
            $('.game-block .game-text').append("<span data-key=\"".concat(key, "\">").concat(value, "<span>")).append(' ');
          } else {
            $('.game-block .game-text').append("<span data-key=\"".concat(key, "\">").concat(value.substr(0, value.length - 1), "<span>")).append(' ');
          }
        });
      }
  ...
  setTimeout(function () {
    ...
    $('.traffic-lights .traffic-light').removeClass('yellow').addClass('green'); // $('.game-block .game-text').empty();
...
function Game(arr, dictionary) {
  ...
  this.countChars = arr.map(function (item) {
    return item.length;
  }).reduce(function (prev, cur) {
    return prev + cur;
  }); //количество символов в массиве

  this.countCharsInWords = arr.map(function (item) {
    return item.length;
  }); //массив, переведенный в массив с количеством символов в элементе исходного массива
  ...
  this.start = function () {
    ...
    if (!isStart) {
      ...
      interval = setInterval(function () {
        ...
        var time = (self.nowTime - self.startTime) / 1000;
  ...
  this.addActive = function () {
    var wordPosition = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : self.wordPosition;
    $("span[data-key=\"".concat(wordPosition, "\"]")).addClass('active');
  }; //удаляет активное слово и мутит


  this.removeActive = function () {
    var wordPosition = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : self.wordPosition;
    $("span[data-key=\"".concat(wordPosition, "\"]")).removeClass('active').addClass('text-muted');
  }; //выделяет слово с ошибкой


  this.addError = function () {
    var wordPosition = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : self.wordPosition;
    $("span[data-key=\"".concat(wordPosition, "\"]")).addClass('error');
  ...
  this.removeError = function () {
    var wordPosition = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : self.wordPosition;
    $("span[data-key=\"".concat(wordPosition, "\"]")).removeClass('error');
      ...
      if (!self.gameStatus) {
        ...
        if (!self.mistake) {
          if (inputValue === self.words[self.wordPosition].substr(0, inputValue.length)) {
            ...
            self.countMistakes++;
            self.addError();
            self.updateKeyboard(); // self.errorKeyboard(self.key);
          }
        ...
        self.errorKeyboard();
        console.log(self.key); // console.log(self.pressedChar);
      }
    });
  };
  ...
  this.endGame = function () {
    ...
    $('.avg-mistakes .mistake').text(self.countMistakes);
    $('.avg-mistakes .percent-mistake').text("(".concat(Math.round(self.percentMistakes), "%)"));
  ...
  this.uploadResultAjax = function () {
    var response;
    var uri = $('.game-stats').data('uri');
    var method = $('.game-stats').data('method');
    $.ajax({
      url: "".concat(uri),
      type: method,
      data: {
      ...
      error: function error(data) {
        console.log(data); //TODO: предложение авторизироваться
      }
    });
  };
  ...
  this.errorKeyboard = function () {
    var key;
    var layout = self.dictionary['language']['lang_code'];
    ...
    if (self.key === 'Backspace') {
      key = 14;
      $(".keyboard-button[name='".concat(key, "'] div")).addClass('active');
      setTimeout(function () {
        $(".keyboard-button[name='".concat(key, "'] div")).removeClass('active');
      }, 160);
    } else {
      if (self.mistake) {
        $(".keyboard-button[name='".concat(key, "'] div")).css('background-color', 'rgb(255, 71, 21)');
        setTimeout(function () {
          $(".keyboard-button[name='".concat(key, "'] div")).removeAttr('style');
        }, 160);
      } else {
        $(".keyboard-button[name='".concat(key, "'] div")).addClass('active');
        setTimeout(function () {
  ...
  this.updateKeyboard = function () {
    ...
    if (key > 0) {
      $(".keyboard-button[name='".concat(key, "']")).addClass('cHover');
    }
